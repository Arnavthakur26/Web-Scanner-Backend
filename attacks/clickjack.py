...
def single_url(url,cors):
    ...
    try:
        ...
        if not sami:
            ...
            results['clickjacking_desc']={'''ClickJacking Vulnerability:
...
'''}
            results['clickjacking_mitigation']={'''Mitigation:
...
''')
            with open(outi,'a') as f:
                results['cors_vuln_urls']={}
                count=0
                f.write(f'{url} [VULNERABLE]')
                results['cors_vuln_urls'].update({count:url})
                count=count+1
                f.close()
