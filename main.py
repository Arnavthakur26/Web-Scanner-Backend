...
def create_folder_if_not_exists(folder_path):
    if not os.path.exists(folder_path):
        ...
        print(Colors.red + "[!] Previous folder removed...")
        quit()
...
def single(targer_url):
    global single_domain
    global protocol
    single_domain = targer_url
    ...
    results['possible_open_redirect_urls']=apply_sed_filter(fil_urls, red_out)
    time.sleep(1)
    ...
    results['possible_xss_urls']=apply_sed_filter(input_file, xss_file)
    time.sleep(1)
    ...
    results['possible_sql_urls']=sql_urls(input_file, sql_file)
    time.sleep(1)
    ...
    results['possible_lfi_urls']=apply_sed_filter(fil_urls, red_out)
    time.sleep(1)
...
def attacks():
    ...
    url = protocol
    results['cors_results']=cors_urls(url)
    print(Colors.cyan + '-----------------------------------------------')
    ...
    results['clickjacking_urls']=single_url(url,cors)
    print(Colors.cyan + '-----------------------------------------------')
    ...
    results['sql_results']=scan_sql_injection(urls, payloads,output_file_path)
    print(Colors.cyan + '\n-----------------------------------------------')
    ...
    results['red_results']=red_url(list_path, payloads_path,output_path)
    print(Colors.cyan + '\n-----------------------------------------------')
    ...
    for domain in domains:
        results['lfi_results']=detect_lfi_vulnerability(domain, payload_list, all_vulnerable_urls,output_file)
    ...
    results['xss_results']=check_xss_vulnerability(url_filename, payload_filename,output_file)
    print(Colors.yellow + '\n-----------------------------------------------\n')
